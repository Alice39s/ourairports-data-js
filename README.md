...
A TypeScript library for working with [OurAirports](https://ourairports.com/) data. This library provides easy access to airport information, including IATA/ICAO codes, geographical coordinates, and more.
...
async function searchAirport(iataCode: string) {
  const airports = new OurAirports();
  ...
  try {
    ...
    const airport = airports.findByIataCode(iataCode);
    if (!airport) {
...
interface BasicInfo {
  ...
  // ... more fields available in ./src/types.ts, see https://github.com/alice39s/ourairports-js/blob/main/src/types.ts
}
```
